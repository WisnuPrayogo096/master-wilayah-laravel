<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900 ml-2">Daftar Kecamatan</h1>
        </div>
                    ...
                    <div class="max-h-[500px] overflow-x-auto relative">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($wilayahs as $kecamatan)
                                <tr class="hover:bg-gray-50 transition-colors duration-200" wire:key="kecamatan-{{ $kecamatan->id }}">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ ($wilayahs->currentPage() - 1) * $wilayahs->perPage() + $loop->iteration }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $kecamatan->kode }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <a wire:navigate href="{{ route('desa', ['kode' => $kecamatan->kode]) }}" class="text-gray-500 hover:text-blue-600 focus:text-blue-600 active:text-blue-700 hover:font-semibold focus:font-semibold active:font-semibold focus:outline-none transition duration-300 ease-in-out">
                                            {{ $kecamatan->nama }}
                                        </a>
                                        <button wire:click="openInfoModal({{ $kecamatan->id }})" wire:key="info-button-{{ $kecamatan->id }}" class="info-button text-blue-600 ml-3 hover:text-blue-800 focus:outline-none transition duration-300 ease-in-out transform hover:scale-110">
                                            <i class="fas fa-info-circle"></i>
                                        </button>
                                    </td>
                                    {{-- <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $kecamatan->ibu_kota }}</td> --}}
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">{{ $kecamatan->type }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">{{ $kecamatan->kategori_wilayah }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">{{ $kecamatan->kode_negara }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">
                                        <button wire:click="openEditModal({{ $kecamatan->id }})" class="update-button text-yellow-600 mr-4 hover:text-yellow-800 focus:outline-none transition duration-300 ease-in-out transform hover:scale-110">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button wire:click="confirmDelete({{ $kecamatan->id }})" class="delete-button text-red-600 hover:text-red-800 focus:outline-none transition duration-300 ease-in-out transform hover:scale-110">
                ...
                @if($showModal)
                    ...
                    >
                        <div class="relative top-20 mx-auto p-8 border w-11/12 max-w-md shadow-2xl rounded-3xl bg-white">
                            ...
                            @if ($modalType === 'info' && $selectedKecamatanId)
                                <h3 class="text-center text-xl font-semibold text-gray-800 mb-6">Info {{ $selectedKecamatan->nama }}</h3>
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @foreach(['created_by', 'updated_by', 'deleted_by', 'restored_by', 'created_at', 'updated_at', 'deleted_at', 'restored_at'] as $field)
                                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $selectedKecamatan->$field }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                            ...
                            @elseif ($modalType == 'delete')
                                ...
                                <div class="flex justify-center space-x-4">
                                    <button wire:click="deleteKecamatan" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-6 rounded-full focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 transition duration-200">
                                        Yes, Delete
                                    </button>
                            ...
                            @elseif($modalType == 'create' || $modalType === 'edit')
                                <h3 class="text-center text-2xl font-semibold text-gray-800 mb-6">
                                    {{ $editingKecamatanId ? 'Edit Kecamatan' : 'Create Kecamatan' }}
                                </h3>

                                <form wire:submit.prevent="{{ $editingKecamatanId ? 'updateKecamatan' : 'createKecamatan' }}" class="space-y-6">
                                    ...
                                    <div class="flex justify-end">
                                        <button type="submit" class="inline-flex justify-center py-2 px-6 border border-transparent shadow-sm text-sm font-medium rounded-full text-white bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-200">
                                            {{ $editingKecamatanId ? 'Update' : 'Create' }}
                                        </button>
                                    </div>
                                </form>
                            @endif
