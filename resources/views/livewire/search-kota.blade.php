<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900 ml-2">Daftar Kota/Kabupaten</h1>
        </div>
            ...
            <div class="p-6" >
                <div class="flex justify-between items-center space-x-4 mb-3">
                    ...
                    <button class="back-button bg-gradient-to-r from-blue-500 to-indigo-600 text-white py-2.5 px-3 rounded-xl ml-2 hover:shadow-lg hover:scale-105 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-50 transition-all duration-300 ease-in-out transform"
                        x-on:click="window.location.href='{{ route('provinsi') }}'">
                    ...
                    <div class="max-h-[500px] overflow-x-auto relative">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($wilayahs as $kota)
                                <tr class="hover:bg-gray-50 transition-colors duration-200" wire:key="kota-{{ $kota->id }}">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ ($wilayahs->currentPage() - 1) * $wilayahs->perPage() + $loop->iteration }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $kota->kode }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <a wire:navigate href="{{ route('kecamatan', ['kode' => $kota->kode]) }}" class="text-gray-500 hover:text-blue-600 focus:text-blue-600 active:text-blue-700 hover:font-semibold focus:font-semibold active:font-semibold focus:outline-none transition duration-300 ease-in-out">
                                            {{ $kota->nama }}
                                        </a>
                                        <button wire:click="openInfoModal({{ $kota->id }})" wire:key="info-button-{{ $kota->id }}" class="info-button text-blue-600 ml-3 hover:text-blue-800 focus:outline-none transition duration-300 ease-in-out transform hover:scale-110">
                                            <i class="fas fa-info-circle"></i>
                                        </button>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $kota->ibu_kota }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">{{ $kota->type }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">{{ $kota->kategori_wilayah }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">{{ $kota->kode_negara }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">
                                        <button wire:click="openEditModal({{ $kota->id }})" class="update-button text-yellow-600 mr-4 hover:text-yellow-800 focus:outline-none transition duration-300 ease-in-out transform hover:scale-110">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button wire:click="confirmDelete({{ $kota->id }})" class="delete-button text-red-600 hover:text-red-800 focus:outline-none transition duration-300 ease-in-out transform hover:scale-110">
                ...
                @if($showModal)
                    ...
                    >
                        <div class="relative top-20 mx-auto p-8 border w-11/12 max-w-md shadow-2xl rounded-3xl bg-white">
                            ...
                            @if ($modalType === 'info' && $selectedKotaId)
                                <h3 class="text-center text-xl font-semibold text-gray-800 mb-6">Info {{ $selectedKota->nama }}</h3>
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @foreach(['created_by', 'updated_by', 'deleted_by', 'restored_by', 'created_at', 'updated_at', 'deleted_at', 'restored_at'] as $field)
                                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $selectedKota->$field }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                            ...
                            @elseif ($modalType == 'delete')
                                ...
                                <div class="flex justify-center space-x-4">
                                    <button wire:click="deleteKota" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-6 rounded-full focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 transition duration-200">
                                        Yes, Delete
                                    </button>
                            ...
                            @elseif($modalType == 'create' || $modalType === 'edit')
                                <h3 class="text-center text-2xl font-semibold text-gray-800 mb-6">
                                    {{ $editingKotaId ? 'Edit Kota/Kabupaten' : 'Create Kota/Kabupaten' }}
                                </h3>

                                <form wire:submit.prevent="{{ $editingKotaId ? 'updateKota' : 'createKota' }}" class="space-y-6">
                                    @foreach(['kode', 'nama', 'ibu_kota'] as $field)
                                        <div>
                                            ...
                                            <input type="text" id="{{ $field }}" wire:model.blur="form.{{ $field }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition duration-200">
