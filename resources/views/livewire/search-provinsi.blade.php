<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900 ml-2">Daftar Provinsi</h1>
        </div>
                    ...
                    <div class="max-h-[500px] overflow-x-auto relative">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($wilayahs as $provinsi)
                                <tr class="hover:bg-gray-50 transition-colors duration-200" wire:key="provinsi-{{ $provinsi->id }}">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ ($wilayahs->currentPage() - 1) * $wilayahs->perPage() + $loop->iteration }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $provinsi->kode }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <a wire:navigate href="{{ route('kota', ['kode' => $provinsi->kode]) }}" class="text-gray-500 hover:text-blue-600 focus:text-blue-600 active:text-blue-700 hover:font-semibold focus:font-semibold active:font-semibold focus:outline-none transition duration-300 ease-in-out">
                                            {{ $provinsi->nama }}
                                        </a>
                                        <button wire:click="openInfoModal({{ $provinsi->id }})" wire:key="info-button-{{ $provinsi->id }}" class="info-button text-blue-600 ml-3 hover:text-blue-800 focus:outline-none transition duration-300 ease-in-out transform hover:scale-110">
                                            <i class="fas fa-info-circle"></i>
                                        </button>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $provinsi->ibu_kota }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">{{ $provinsi->type }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">{{ $provinsi->kategori_wilayah }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">{{ $provinsi->kode_negara }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center align-middle text-sm text-gray-500">
                                        <button wire:click="openEditModal({{ $provinsi->id }})" class="update-button text-yellow-600 mr-4 hover:text-yellow-800 focus:outline-none transition duration-300 ease-in-out transform hover:scale-110">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button wire:click="confirmDelete({{ $provinsi->id }})" class="delete-button text-red-600 hover:text-red-800 focus:outline-none transition duration-300 ease-in-out transform hover:scale-110">
                    ...
                    >
                        <div class="relative top-20 mx-auto p-8 border w-11/12 max-w-md shadow-2xl rounded-3xl bg-white">
                            ...
                            @if ($modalType === 'info' && $selectedProvinsiId)
                                <h3 class="text-center text-xl font-semibold text-gray-800 mb-6">Info {{ $selectedProvinsi->nama }}</h3>
